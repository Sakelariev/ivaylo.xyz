...
author:
  - name: Ивайло Сакелариев
    url: {}
preview: static/cherry_grape.png
date: 12-23-2021
output:
...
temperature_graph <- ggplot(cherries_temp, aes(x=Година, y=Температура)) +
  ...
  labs(x = "Година", y="Средна априлска температура в Киото", family = "sans serif")
```
...
Повечето българи знаят, че зреенето на гроздето зависи от няколко очевидни фактора - климатични (температура, валежи, влажност) и промяна в земеделските методи. Също така обаче по-ранното узряване на гроздето (съответно по-ранното му бране) може да се обясни и с промяната на сортовете, които се отглеждат или промени в тяхната фенология. Следователно отново не можем
